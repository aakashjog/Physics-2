...
\begin{figure}[H]
	\begin{tikzpicture}
		...
		\draw (\R,\d/2) arc (0:180:\R);
		\draw (-\R,\d/2) -- (\R,\d/2);

		\draw (-\R,-\d/2) arc (180:360:\R);
		\draw (-\R,-\d/2) -- (\R,-\d/2);
		...
		\foreach \r in {1,...,\R}
		{
			\draw (particle) -- ++($ (0,\d) - (0,2pt) $);
			\draw ($ (particle) + (0,\d) - (0,2pt) $) arc (0:180:\r);

			\coordinate (particle) at ($ (particle) + (0,\d) - (0,2pt) + (-2*\r,0) $);
		}
	\end{tikzpicture}<++>
\end{figure}<++>
