...
\begin{question}
	The electric potential is given in cylindrical coordinates by
	\begin{equation*}
		V(r) =
			\begin{cases}
				A (a^2 - r^2)                    & ;\quad r < a     \\
				-A a^2 \ln \frac{r}{a} + C       & ;\quad a < r < b \\
				-(A a^2 + B) \ln \frac{r}{b} + D & ;\quad b < r     \\
			\end{cases}
	\end{equation*}
	...
	\begin{enumerate}
		\item Calculate the electric field everywhere.
		\item Find the charge distributions that create this field.
		\item Find the surface charge density at $r = a$ and at $r = 2 b$.
		\item Write an expression for $C$ and $D$ as functions of the given parameters $a$, $b$, $A$ and $B$.
	\end{enumerate}
\end{question}
...
\begin{question}
	A parallel plate capacitor is made of two circular disks of radius $a$ with distance $d$ between them.
	The capacitor carries charge $\pm Q_0$ on its plates.
	At $t = 0$, the capacitor is connected to a resistor $R$.
	\begin{enumerate}
		\item Find the time dependent current $I(t)$, in the circuit.
		\item Find the surface current density $\overrightarrow{k}(r,t)$, on the plates of the capacitor.
	\end{enumerate}
	The capacitor is charged again, with charge $\pm Q_0$ on its plates.
	At $t = 0$, the capacitor is filled with a material of resistivity $\rho_0$.
	\begin{enumerate}[resume]
		\item Find the time dependent current $I(t)$, in the circuit.
		\item Find the surface current density $\overrightarrow{k}(r,t)$, on the plates of the capacitor.
	\end{enumerate}
...
\begin{question}
	A square loop of sides $a$ and mass $m$, carrying current $I$, is hung on the $z$ axis with frictionless hinges as shown.
	\begin{figure}[H]
		\begin{tikzpicture}
			\def\xMIN{-3};
			\def\xMAX{3};
			...
			\coordinate (loop top) at (0,0);
			\coordinate (loop bottom) at ({-90 + \angle}:\a);
			\coordinate (loop center) at ({-90 + \angle}:{\a/2});
			...
			\begin{scope}[ultra thick]
				\draw (loop top) -- (loop bottom);
				\node at (loop top) {$\odot$};
				\node at (loop bottom) {$\otimes$};
			\end{scope}

			\begin{scope}[-stealth]
				\foreach \x in {\xMIN,...,\xMAX}
				{
					\draw (\x,\yMIN) -- (\x,\yMAX) node [above] {$B$};
				}
			\end{scope}
		\end{tikzpicture}
	\end{figure}
	The gravitational force is in the $-\hat{y}$ direction.
	Calculate the magnetic field $\overrightarrow{B} = B_y \hat{y}$ needed in order to maintain the square loop at angle $\theta$ with the $y$-$z$ plane.
\end{question}
...
\begin{solution}
	\begin{figure}[H]
		\begin{tikzpicture}
			...
			\begin{scope}[-stealth]
				\draw (loop center) -- ++(-90:\F) node [below] {$F_g$};
	...
	\end{figure}
	\begin{align*}
		\overrightarrow{\tau_g} &= \overrightarrow{r} \times \overrightarrow{F_g}\\
		&= \left( \frac{a}{2} \right) \left( \hat{i} \sin \theta - \hat{j} \cos \theta \right) \times \left( m g \right) \left( -\hat{j} \right)\\
		&= \frac{a}{2} \cdot m g \sin \theta \left( -\hat{k} \right)
	\end{align*}
	\begin{align*}
		\overrightarrow{\tau_B} &= \overrightarrow{r} \times \overrightarrow{F_B}\\
		&= a \left( \hat{i} \sin \theta - \hat{j} \cos \theta \right) \left( I a B_0 \hat{i} \right)\\
		&= I a^2 B \cos \theta \hat{k}
	\end{align*}
	For the rod to be at equilibrium,
	\begin{align*}
		\tau_g &= \tau_B\\
		\therefore B &= \frac{m g}{2 I a} \tan \theta
	\end{align*}
\end{solution}
...
\begin{question}
	A hollow cylinder of inner radius $a$ and outer radius $b$ is given.
	The following magnetic field is measured
	\begin{equation*}
		B =
			\begin{cases}
				0 \hat{\theta}                                  & ;\quad r < a     \\
				C \left( r - \frac{a^2}{r} \right) \hat{\theta} & ;\quad a < r < b \\
				\frac{A}{r} \hat{\theta}                        & ;\quad b < r     \\
			\end{cases}
	\end{equation*}
	where $r$ is the distance from the $z$ axis.
	The constants $A$ and $C$ are also given.\\
	Calculate the current density $\overrightarrow{j}$ everywhere.
\end{question}
...
\begin{solution}
	\begin{figure}[H]
		\begin{tikzpicture}
			...
			\begin{scope}
				\draw (\xMAX, -\b/2) -- (0,-\b/2) -- (0,\b/2) -- (\xMAX,\b/2);
			\end{scope}

			\begin{scope}[xshift = -10]
				\draw [decorate, decoration = {mirror, brace}] (0,\b/2) -- (0,-\b/2) node [midway, left] {$b$};
			\end{scope}
		\end{tikzpicture}
	\end{figure}
		\begin{align*}
...
\addtocounter{question}{1}
\begin{question}
	The magetic field inside an infinite long solenoid of radius $a$ is $\overrightarrow{B} = B_z(t) \hat{z}$.
	\begin{enumerate}
		\item Calculate the electric field everywhere.
		\item Show that the differential and the integral forms of Faraday's Law give the same result.
	\end{enumerate}
\end{question}
...
\begin{solution}
	...
	\begin{align*}
		\Phi_B &=
			\begin{cases}
				\mu_0 \pi r^2 n I(t) & ;\quad r < a \\
				\mu_0 \pi a^2 n I(t) & ;\quad r > a \\
			\end{cases}
	\end{align*}
	Therefore,
	\begin{align*}
		\dod{\Phi_B}{t} &=
			\begin{cases}
				\mu_0 \pi r^2 n \dot{I}(t) & ;\quad r < a \\
				\mu_0 \pi a^2 n \dot{I}(t) & ;\quad r > a \\
			\end{cases}
	\end{align*}
	Therefore, by the integral form of Faraday's Law,
	\begin{align*}
		\oint \overrightarrow{E} \cdot \overrightarrow{\dif l} &= -\dod{\Phi_B}{t}\\
		\therefore E \cdot 2 \pi r                             &=
			\begin{cases}
				-\mu_0 \pi r^2 n \dot{I}(t) & ;\quad r < a \\
				-\mu_0 \pi a^2 n \dot{I}(t) & ;\quad r > a \\
			\end{cases}\\
...
\begin{solution}
	If the current in the larger solenoid is $I_2$ and the current in the smaller solenoid is $0$,
	\begin{align*}
		B & = \mu_0 n_2 I_2
	\end{align*}
	Therefore, the magnetic flux through the short solenoid is,
	\begin{align*}
		{\Phi_B}_1 & = n_1 l B A \\
                           & = n_1 l \cdot \mu_0 n_2 I_2 \cdot \pi a^2
	\end{align*}
	As $M = \frac{{\Phi_B}_1}{I_2} = \frac{{\Phi_B}_2}{I_1}$,
	\begin{align*}
		\frac{{\Phi_B}_2}{I_1}            & = \frac{{\Phi_B}_1}{I_2}                              \\
		\therefore \frac{{\Phi_B}_2}{I_1} & = \frac{n_1 l \cdot \mu_0 n_2 I_2 \cdot \pi a^2}{I_2} \\
                                                  & = \mu_0 \pi n_1 n_2 l a^2                             \\
		\therefore {\Phi_B}_2             & = \mu_0 \pi n_1 n_2 l a^2 I_1
	\end{align*}
	Therefore, the flux through the long solenoid is $\mu_0 \pi n_1 n_2 l a^2 I_1$.
\end{solution}

\recitation

\begin{question}
	A coaxial cable of length $L$ is made out of two parallel, conductive, thin tubes of radii $a,b << L$, each carrying current $I$ in the opposite direction.
	\begin{enumerate}
		\item Calculate the magnetic field everywhere.
		\item Calculate the cable's inductance.
		\item Calculate the magnetic energy density.
		\item Using the energy density, calculate the cable's inductance.
	\end{enumerate}
\end{question}
